            ...
            for (int blockRow = i; blockRow < i + blockSize; blockRow++){
                for (int blockCol = j; blockCol < j+blockSize; blockCol++){
                    for (int k = 0; k<Loop_Before_Remainder; k+=4){
                        //input 0
                        float temp_vector0[4];
                        temp_vector0[0] = (*input0).data[blockRow*col_size_input0+(k+0)];
                        temp_vector0[1] = (*input0).data[blockRow*col_size_input0+(k+1)];
                        temp_vector0[2] = (*input0).data[blockRow*col_size_input0+(k+2)];
                        temp_vector0[3] = (*input0).data[blockRow*col_size_input0+(k+3)];
                        float32x4_t vector0 = vld1q_f32(temp_vector0);
                    
                        //input 1
                        float temp_vector1[4];
                        temp_vector1[0] = (*input1).data[(k+0)*col_size_input1+blockCol];
                        temp_vector1[1] = (*input1).data[(k+1)*col_size_input1+blockCol];
                        temp_vector1[2] = (*input1).data[(k+2)*col_size_input1+blockCol];
                        temp_vector1[3] = (*input1).data[(k+3)*col_size_input1+blockCol];
                        float32x4_t vector1 = vld1q_f32(temp_vector1);
                    
                        float32x4_t data = vmulq_f32(vector0, vector1);
                        result->data[blockRow*col_size_input1+blockCol] += vaddvq_f32(data);
                        //(*result).data[blockRow*col_size_input1+blockCol] += vaddvq_f32(result);
                    }
                    if (remainder != 0){
                        //Haven't started this with neon commands yet...
                        for (int k = Loop_Before_Remainder; k<col_size_input0; k++){
                            float temp_vector0[4] = {0,0,0,0};
                            temp_vector0[0] = (*input0).data[blockRow*col_size_input0+k];
                            float32x4_t vector0 = vld1q_f32(temp_vector0);

                            float temp_vector1[4] = {0,0,0,0};
                            temp_vector1[0] = (*input1).data[k*col_size_input1+blockCol];
                            float32x4_t vector1 = vld1q_f32(temp_vector1);
